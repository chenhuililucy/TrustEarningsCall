...
# ---------------------------------------------------------------------
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            if self.financials_agent and "InspectPastStatements" in buckets:
                tasks.append(executor.submit(
                    lambda: ("financials", self.financials_agent.run(buckets["InspectPastStatements"], row))
                ))
    ...
    def summarise(self, items: List[Dict[str, Any]]) -> str:
        """
        Produce ONE Up/Down call + confidence using the 3 batch notes
        generated in `delegate`.
        """
        notes = {
        ...
        final_prompt = main_agent_prompt(notes)
    ...
    def run(self, row: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Peer discovery (best‑effort)
        try:
            peers_resp = self._chat(f"Give 3 peer tickers for {row['ticker']} as JSON list of strings.")
        ...
        # Summarise
        notes, decision = self.summarise(facts)
        return {"items": facts, "notes": notes, "summary": decision}
